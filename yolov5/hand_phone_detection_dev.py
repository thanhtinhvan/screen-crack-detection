...
@torch.no_grad()
def run(weights=ROOT / 'last.pt',  # model.pt path(s) ROOT / 'yolov5s.pt'
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for c in det[:, -1].unique():
                    ...
                    cnt[names[
                        int(c)]] = n  # n is the total number of detections detected per class eg. 2 phones or 5 phones etc.
                    # print('\ndetections are :::::: {}'.format(det[:, :4]))
                    print('\n cnt dict is :::: {}'.format(cnt))
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        cv2.rectangle(im0, (x1, y1), (x2, y2), (0, 0, 255),
                                      3)  # Drawing the rectangular box on the screen 16:9 ratio for phone
                        cv2.circle(im0, (int(x1 + focus_x), int(y1 + focus_y)), 4, (0, 0, 255),
                                   -1)  # fixed center point

                        if names[c] == 'phone':
                            storephonedata = [int(xyxy[0]), int(xyxy[1]), int(xyxy[2]), int(xyxy[3])]
                            storephoneoriginalcords = xyxy
                            print('phone ka xyxy is ', xyxy)
                            cv2.circle(im0, (int(centrx), int(centry)), 4, (255, 0, 0),
                                       -1)  # fixed center point of the bounding box
                            # print('coords are these',int(xyxy[0]), int(xyxy[1]), type(xyxy[0]))
                            # print('Centerx & Centery is ', int(centrx.cpu().numpy()), int(centry.cpu().numpy()))
                            # print('second points are ==', int(x1+focus_x), int(y1+focus_y))
                        ...
                        # region_one = np.array([[x1+40,y1+40], 
                        #                            [x1+40,x2-40],
                        #                            [x2-40,y2-40],
                        #                            [x2-40,y1+40]], np.int32)
                        # region_one = region_one.reshape((-1,1,2))
                        # cv2.polylines(im0, [region_one], True, (255, 255, 255), 3) # Drawing the area ie.. polygon
                        if names[c] == 'finger':
                            # cv2.circle(im0, (int(centrx+0.5*centrw), int(centry)), 4, (255,0,0), -1) # fixed center point of the bounding box
                            # cv2.circle(im0, (int(centrx-0.5*centrw), int(centry)), 4, (255,0,0), -1) # fixed center point of the bounding box
                            # cv2.circle(im0, (int(centrx), int(centry)), 4, (255,0,0), -1) # fixed center point of the finger bounding box
                            ...
                            # inside_region_one = cv2.pointPolygonTest(region_one, (int(centrx), int(centry)), False)
                            # inside_region_two = cv2.pointPolygonTest(region_one, (int(centrx), int(centry)), False)

                            # xx1, yy1, xx2, yy2 = max(x1,int(xyxy[0])), max(y1, int(xyxy[1])), min(x2, int(xyxy[2])), min(y2, int(xyxy[3]))
                            # intersection = np.clip((xx2-xx1), 0,1)*np.clip((yy2-yy1),0,1)
                            # box1_area = abs((x2-x1) * (y1-y2))
                            # box2_area = abs((int(xyxy[2])-int(xyxy[0])) * (int(xyxy[1])-int(xyxy[3])))

                            # iou = intersection/(box1_area+box2_area-intersection)
                            # print(float(iou))
                            # cv2.putText(im0, f'{str(float(iou))}', (10, im0.shape[0]-420), cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.7, color=(0, 0, 255), thickness=2)

                            # if inside_region_one:
                            #     cv2.putText(im0, 'Please move your finger out of phone', (10, im0.shape[0]-420), cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.7, color=(0, 0, 255), thickness=2)
            ...
            # cv2.rectangle(im0, (x1, y1), (x2, y2), (0,255,0), 3) # Drawing the rectangular box on the screen 16:9 ratio for phone
            ...
            if len(storephonedata):
                ...
                if True in fingercheck:
                    cv2.putText(im0, 'Please move your finger out of phone!!!!!!', (8, im0.shape[0] - 420),
                                cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.7, color=(0, 0, 255), thickness=2)

                if True not in fingercheck and euclidean_dist and euclidean_dist < 20:
                    print("\n............now saving the file.......")
                    cv2.putText(im0, 'ALL OKKKKKKKKKKKKKK', (8, im0.shape[0] - 20), cv2.FONT_HERSHEY_SIMPLEX,
                                fontScale=0.7, color=(0, 255, 0), thickness=2)
                    save_one_box(storephoneoriginalcords, imc, file=save_dir / 'crops' / 'phoneonly' / f'{p.stem}.jpg',
                                 BGR=True)
                    return save_dir / 'crops' / 'phoneonly' / f'{p.stem}.jpg'
